...
class AudioCapture:
    ...
    def _find_macos_loopback(self) -> int:
        ...
        Falls back to the built-in microphone when BlackHole is not installed.

        Returns:
            Device ID for BlackHole or fallback microphone.
        """
        devices = sd.query_devices()

        # First, try to find BlackHole
        for i, device in enumerate(devices):
            if "blackhole" in device["name"].lower():
                if device["max_input_channels"] > 0:
                    print(f"BlackHole encontrado: [{i}] {device['name']}")
                    return i

        # BlackHole not found — fall back to built-in microphone
        print(
            "\nBlackHole nao encontrado — usando microfone como fallback.\n"
            "Para capturar audio do sistema, instale o BlackHole:\n"
            "https://existential.audio/blackhole/\n"
        )

        for i, device in enumerate(devices):
            name = device["name"].lower()
            if ("macbook" in name or "built-in" in name or "microfone" in name) \
                    and device["max_input_channels"] > 0:
                print(f"Usando microfone: [{i}] {device['name']}")
                return i
        ...
        raise RuntimeError("Nenhum dispositivo de entrada de audio encontrado.")
